...

{
  ...
  programs.zsh = {
    ...
    shellAliases = {
      ...
      update = "bash ~/nixos-config/pkgs/v2ray-rules-dat/update-v2ray-rules-dat.sh && nix flake update opencode-config --flake ~/nixos-config && sudo nixos-rebuild switch --flake ~/nixos-config#dev-machine --impure";
      # 清理 Nix 垃圾回收
