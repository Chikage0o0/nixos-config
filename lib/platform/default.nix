...
let
  ...
  requireAttrs = attrPath: value: names:
    if names == [ ] then
      true
    else
      let
  ...
  normalizeHost = host:
    ...
    in
    builtins.seq _requiredTop (builtins.seq _requiredUser (
    host
    // {
  ...
  sopsModule = host: { lib, ... }:
    ...
    lib.mkIf (sops.enable or false) {
      sops.defaultSopsFile = sops.defaultFile or (throw "mkHost requires host.secrets.sops.defaultFile when secrets.sops.enable = true");
      sops.age.keyFile = sops.ageKeyFile or (throw "mkHost requires host.secrets.sops.ageKeyFile when secrets.sops.enable = true");
